...
class TDB_Mysqli {
	...
	public function __construct($host,$user,$pass,$name) {
		...
		if ($this->handler->connect_error) {
			throw new EDatabaseError("Imposible conectar a la base de datos: {$this->handler->connect_error} ({$this->handler->connect_errno})");
		}
	}
	...
	public function getRecords($table,$condition="",$order="",$start=-1,$number=-1,$debug=false) {
		...
		$query="select {$DISTINCT}SQL_CALC_FOUND_ROWS {$fields} from {$table} {$where} {$order} {$limit}";
		if ($debug) {
			$content="";
			if (file_exists("/tmp/database_sql.log"))
				$content=file_get_contents("/tmp/database_sql.log");
			file_put_contents("/tmp/database_sql.log",$content.date("Y-m-d H:i:s").": {$query}\n");
		}
